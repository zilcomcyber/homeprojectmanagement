...
function check_user_agent() {
    if (isset($_SESSION['user_agent'])) {
        if ($_SESSION['user_agent'] !== $_SERVER['HTTP_USER_AGENT']) {
            ...
            logout_user();
            $redirect_url = (strpos($_SERVER['REQUEST_URI'], '/admin/') !== false) ? '../admin.php?error=security' : 'admin.php?error=security';
...
function require_role($required_role) {
    ...
    if (!has_role($required_role)) {
        ...
        $redirect_url = (strpos($_SERVER['REQUEST_URI'], '/admin/') !== false) ? '../admin.php?error=insufficient_permissions' : 'admin.php?error=insufficient_permissions';
        header('Location: ' . $redirect_url);
        exit;
    }
