...
function add_project_rating($project_id, $rating, $user_name = null, $user_email = null, $comment = null) {
    ...
    try {
        ...
        // Check if user has already rated (by IP for anonymous users)
        $ip_address = $_SERVER['REMOTE_ADDR'] ?? '';
        $stmt = $pdo->prepare("SELECT id FROM project_ratings WHERE project_id = ? AND ip_address = ?");
        $stmt->execute([$project_id, $ip_address]);
        ...
        // Insert the rating
        $stmt = $pdo->prepare("INSERT INTO project_ratings (project_id, rating, user_name, user_email, comment, ip_address) VALUES (?, ?, ?, ?, ?, ?)");
...
function calculate_project_progress($project_id) {
    ...
    try {
        ...
        $stmt = $pdo->prepare("SELECT COUNT(*) FROM project_steps WHERE project_id = ? AND status = 'completed'");
        $stmt->execute([$project_id]);
