...
try {
    ...
    while (($data = fgetcsv($handle)) !== false) {
        ...
        $budget = trim($data[8]);
        $project_year = trim($data[9]);
        $start_date = trim($data[10]);
        $expected_completion_date = trim($data[11]);
        $contractor_name = trim($data[12]);
        $contractor_contact = trim($data[13]);
        $step_name = trim($data[14]);
        $step_description = trim($data[15]);
        ...
        try {
            ...
            if (!$department_id) {
                $errors[] = "Row {$row_number}: Department '{$department_name}' not found";
                $error_count++;
                continue;
            }
            ...
            if (!$county_id) {
                $errors[] = "Row {$row_number}: County '{$county_name}' not found";
                $error_count++;
                continue;
            }
            ...
            if (!$sub_county_id) {
                $errors[] = "Row {$row_number}: Sub-county '{$sub_county_name}' not found";
                $error_count++;
                continue;
            }
            ...
            if (!$ward_id) {
                $errors[] = "Row {$row_number}: Ward '{$ward_name}' not found";
                $error_count++;
                continue;
            }
            ...
            $budget_amount = 0;
            if (!empty($budget)) {
                $budget_amount = floatval(str_replace(',', '', $budget));
            }

            // Insert project
            $project_sql = "INSERT INTO projects (
                project_name, description, department_id, county_id, sub_county_id, ward_id,
                location_address, location_coordinates, budget, project_year, start_date,
                expected_completion_date, contractor_name, contractor_contact, status,
                total_steps, completed_steps, progress_percentage, created_by, created_at
