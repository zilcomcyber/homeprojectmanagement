    ...
    if (!verify_csrf_token($_POST['csrf_token'] ?? '')) {
        ...
        switch ($action) {
            ...
            case 'update_visibility':
                ...
                break;
            case 'delete_project':
                $result = delete_project_from_manage($_POST);
                break;
            default:
...
function delete_project_from_manage($data) {
    global $pdo, $project_id, $current_admin;
    ...
    try {
        ...
        // Delete project steps first (due to foreign key constraints)
        $stmt = $pdo->prepare("DELETE FROM project_steps WHERE project_id = ?");
        $stmt->execute([$project_id]);

        // Delete feedback related to this project
...
<div class="flex justify-between items-center mb-6">
    <div>
        <h2 class="text-2xl font-bold text-gray-900 dark:text-white">Manage Project</h2>
        <p class="text-gray-600 dark:text-gray-400">Project: <?php echo htmlspecialchars($project['project_name']); ?></p>
    </div>
    <div class="flex space-x-2">
        <a href="edit_project.php?id=<?php echo $project['id']; ?>" class="inline-flex items-center px-3 py-2 border border-gray-300 dark:border-gray-600 text-sm font-medium rounded-md text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 transition-colors">
            <i class="fas fa-edit mr-2"></i>
            Edit Project
        </a>
        <form method="POST" style="display: inline;" onsubmit="return confirm('Are you sure you want to delete this project? This action cannot be undone and will also delete all associated steps and feedback.')">
            <input type="hidden" name="csrf_token" value="<?php echo generate_csrf_token(); ?>">
            <input type="hidden" name="action" value="delete_project">
            <button type="submit" class="inline-flex items-center px-3 py-2 border border-red-600 text-sm font-medium rounded-md text-red-600 hover:bg-red-50 dark:hover:bg-red-900 transition-colors">
    ...
    <div class="p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div>
                ...
                <dd class="text-sm text-gray-900 dark:text-white"><?php echo htmlspecialchars($project['department_name']); ?></dd>
            </div>
            <div>
                <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Location</dt>
                <dd class="text-sm text-gray-900 dark:text-white"><?php echo htmlspecialchars($project['ward_name'] . ', ' . $project['sub_county_name']); ?></dd>
            </div>
...
<div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 mb-6">
    <div class="p-6 border-b border-gray-200 dark:border-gray-700">
        <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Project Visibility</h3>
        <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">Control public access to this project</p>
    </div>
    <div class="p-6">
        <form method="POST" class="flex items-center justify-between">
            ...
            <div class="flex items-center space-x-3">
                <select name="visibility" class="px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md text-sm focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white">
...
<div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
    ...
    <?php if (empty($steps)): ?>
        <div class="p-12 text-center">
            ...
            <p class="text-gray-600 dark:text-gray-400 mb-4">This project doesn't have any steps yet.</p>
            <button onclick="showAddStepModal()" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 transition-colors">
                <i class="fas fa-plus mr-2"></i>
                Add First Step
            </button>
                    ...
                    <div class="flex items-start justify-between">
                        <div class="flex-1">
                            <div class="flex items-center mb-2">
                                ...
                                <span class="ml-3 inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                    <?php 
                                    echo $step['status'] === 'completed' ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300' : 
                                        ($step['status'] === 'in_progress' ? 'bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-300' : 
                                        ($step['status'] === 'skipped' ? 'bg-gray-100 text-gray-800 dark:bg-gray-900 dark:text-gray-300' : 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-300'));
                                    ?>">
                            ...
                            <?php if ($step['description']): ?>
                                <p class="text-sm text-gray-600 dark:text-gray-400 mb-3 ml-11">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm ml-11">
                                <?php if ($step['start_date']): ?>
                                    <div>
                                        ...
                                        <span class="text-gray-900 dark:text-white"><?php echo format_date($step['start_date']); ?></span>
                                    </div>
                                <?php endif; ?>
                                
                                <?php if ($step['expected_end_date']): ?>
                                    <div>
                                        <span class="font-medium text-gray-500 dark:text-gray-400">Expected:</span>
                                        <span class="text-gray-900 dark:text-white"><?php echo format_date($step['expected_end_date']); ?></span>
                                    </div>
                                <?php endif; ?>
                                ...
                                <?php if ($step['actual_end_date']): ?>
                                    <div>
                                        ...
                                        <span class="text-gray-900 dark:text-white"><?php echo format_date($step['actual_end_date']); ?></span>
                                    </div>
                                <?php endif; ?>
                            </div>
                            ...
                            <?php if ($step['notes']): ?>
                                <div class="mt-3 ml-11">
                                    <span class="font-medium text-gray-500 dark:text-gray-400 text-sm">Notes:</span>
                                    <p class="text-sm text-gray-900 dark:text-white mt-1"><?php echo htmlspecialchars($step['notes']); ?></p>
                                </div>
                            <?php endif; ?>
                        </div>
                        ...
                        <div class="flex items-center space-x-2 ml-4">
                            <form method="POST" class="inline">
                                ...
                                <select name="status" onchange="this.form.submit()" class="text-sm border border-gray-300 dark:border-gray-600 rounded-md px-2 py-1 dark:bg-gray-700 dark:text-white">
                            ...
                            <button onclick="editStep(<?php echo htmlspecialchars(json_encode($step)); ?>)" class="text-blue-600 dark:text-blue-400 hover:text-blue-500" title="Edit Step">
                            ...
                            <form method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this step?')">
                                ...
                                <input type="hidden" name="step_id" value="<?php echo $step['id']; ?>">
                                <button type="submit" class="text-red-600 dark:text-red-400 hover:text-red-500" title="Delete Step">
    ...
    <div class="flex items-center justify-center min-h-screen p-4">
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-md w-full">
            <form method="POST">
                ...
                <div class="p-6">
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Add New Step</h3>
...
<script>
    ...
    // Delete project function is handled by admin.js
</script>
