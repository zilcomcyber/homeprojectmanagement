...
if (isset($_GET['id'])) {
    ...
    // Parse clean URL format /project_details/123
...
error_log("Project ID: $project_id, Comments found: " . count($project_comments) . ", Approved: " . $approved_comments_count);
...
@media (max-width: 768px) {
    ...
    .replies-container {
        margin-left: 1.5rem; /* Reduced from ml-12 */
    }
    ...
    /* Ensure text doesn't break into single characters */
    .text-gray-700, .comment-text-compact {
...
/* Enhanced glass cards with white translucent background */
.glass-card {
    position: relative;
    overflow: hidden;
...
/* Enhanced hover effects for related projects */
.group:hover .circular-progress {
    transform: scale(1.05);
    transition: transform 0.3s ease;
}

/* Pulsing animation for active projects */
...
.scroll-indicator {
    ...
    width: 100%;
    height: 4px;
    background-color: rgba(226, 232, 240, 0.8); /* Light gray background */
...
async function loadMoreReplies(parentId, currentOffset) {
    ...
    try {
        const response = await fetch(`${window.BASE_URL}api/load_more_replies.php?parent_id=${parentId}&offset=${currentOffset}&limit=5`);
