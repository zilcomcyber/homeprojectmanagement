// Admin-specific JavaScript functionality for County Project Tracking System
...
class AdminManager {
    ...
    initProjectForm() {
        ...
        window.loadSubCounties = (countyId) => {
            ...
            fetch(`../api/locations.php?type=sub_counties&county_id=${countyId}`)
                .then(response => response.json())
                .then(data => {
                    this.populateSelect('subCountyId', data, 'id', 'name', 'Select Sub County');
                    this.clearSelect('wardId');
                })
                .catch(error => {
        ...
        window.loadWards = (subCountyId) => {
            ...
            fetch(`../api/locations.php?type=wards&sub_county_id=${subCountyId}`)
                .then(response => response.json())
                .then(data => {
                    this.populateSelect('wardId', data, 'id', 'name', 'Select Ward');
                })
                .catch(error => {
    ...
    async uploadCSV() {
        ...
        try {
            ...
            const response = await fetch('../api/upload_csv.php', {
                method: 'POST',
                body: formData
            });
...
function downloadSampleCSV() {
    const csvContent = `project_name,description,department,ward,sub_county,county,budget,year,status,progress_percentage,contractor_name,start_date,expected_completion_date,location_coordinates,location_address
New Water Plant,Construction of new water treatment plant,Water and Sanitation,Central Ward,Nairobi Central,Nairobi,50000000,2024,ongoing,45,ABC Contractors,2024-01-15,2024-12-31,-1.2921,36.8219,123 Main Street Nairobi
Road Construction,Tarmacking of rural roads,Roads and Transport,West Ward,Kiambu East,Kiambu,25000000,2024,planning,0,XYZ Construction,2024-03-01,2024-11-30,-1.1744,36.9482,Rural Road Network
Health Center,Construction of new health center,Health Services,Health Services,North Ward,Nakuru East,Nakuru,30000000,2023,completed,100,Health Builders Ltd,2023-01-01,2023-12-15,-0.3031,36.0800,Hospital Road Nakuru`;
...
function populateProjectForm(project) {
    ...
    document.getElementById('allocatedBudget').value = project.allocated_budget || '';
    document.getElementById('spentBudget').value = project.spent_budget || '';
